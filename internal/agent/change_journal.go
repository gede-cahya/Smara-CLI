...
func (s *Supervisor) captureChangeJournal(userPrompt string, result *PromptResult, trace []skill.TraceStep) {
...
func hasMeaningfulChange(tools []string, trace []skill.TraceStep) bool {
...
func buildChangeJournalEntry(userPrompt string, result *PromptResult, trace []skill.TraceStep, mode Mode) string {
...
func extractChangeTargets(trace []skill.TraceStep) []string {
...
func cloneTraceSteps(trace []skill.TraceStep) []skill.TraceStep {
	cloned := make([]skill.TraceStep, 0, len(trace))
	for _, step := range trace {
		args := make(map[string]interface{}, len(step.Args))
		for k, v := range step.Args {
			args[k] = v
		}
		cloned = append(cloned, skill.TraceStep{Tool: step.Tool, Args: args})
	}
	return cloned
}
