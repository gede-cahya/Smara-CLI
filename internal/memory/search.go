...
// searchByEmbedding performs vector similarity search using cosine similarity.
// This is a brute-force approach for MVP; can be replaced with sqlite-vec later.
func searchByEmbedding(db *sql.DB, queryEmbedding []float32, topK int) ([]SearchResult, error) {
	if topK <= 0 {
		topK = 5
	}

	rows, err := db.Query(
		"SELECT id, content, embedding, tags, source, created_at FROM memories WHERE embedding IS NOT NULL",
	)
	if err != nil {
