...
func (m *NodeGraphModel) UpdateAgent(id, role string, status NodeStatus, task string) {
	if n, ok := m.nodes[role]; ok {
		n.ID = id; n.Status = status; n.Task = task
		m.nodes[role] = n
	}
}

func (m *NodeGraphModel) SetNodeProgress(role string, progress float64) {
	if n, ok := m.nodes[role]; ok { n.Progress = progress; m.nodes[role] = n }
}

func (m *NodeGraphModel) AddEdge(from, to, label string, active bool) {
	for i, e := range m.edges {
		if e.From == from && e.To == to {
			m.edges[i].Active = active; m.edges[i].Label = label
			return
		}
	}
...
func (m *NodeGraphModel) FocusNext() {
	if len(m.nodeKeys) == 0 { return }
	m.focusedNode = (m.focusedNode + 1) % len(m.nodeKeys)
}
func (m *NodeGraphModel) FocusPrev() {
	if len(m.nodeKeys) == 0 { return }
	m.focusedNode--
	if m.focusedNode < 0 { m.focusedNode = len(m.nodeKeys)-1 }
}
func (m *NodeGraphModel) FocusedRole() string {
	if m.focusedNode < 0 || m.focusedNode >= len(m.nodeKeys) { return "" }
	return m.nodeKeys[m.focusedNode]
}
func (m *NodeGraphModel) TogglePopup() {
	if m.showPopup { m.showPopup = false; m.popupNode = "" } else { m.showPopup = true; m.popupNode = m.FocusedRole() }
}
...
func (m NodeGraphModel) View() string {
	if m.width < 40 || m.height < 10 { return "Terminal too small" }
	...
	type cell struct { char rune; style lipgloss.Style; z int }
	canvas := make([][]cell, m.height)
	for y := range canvas {
		canvas[y] = make([]cell, m.width)
		for x := range canvas[y] { canvas[y][x] = cell{char:' '} }
	}
	...
	for _, edge := range m.edges { m.drawEdge(edge, placeRune, 1) }
	...
	for y := 0; y < m.height; y++ {
		...
		for x := 0; x < m.width; x++ {
			c := canvas[y][x]
			if c.z > 0 { line.WriteString(c.style.Render(string(c.char))) } else { line.WriteRune(c.char) }
		}
	...
	if m.showPopup && m.popupNode != "" {
		if n, ok := m.nodes[m.popupNode]; ok { return m.overlayPopup(sb.String(), m.renderPopup(n)) }
	}
	return sb.String()
}
...
func (m NodeGraphModel) drawNode(node NodeState, pos NodePosition, focused bool, place func(int,int,string,lipgloss.Style,int), placeRune func(int,int,rune,lipgloss.Style,int), z int) {
	...
	if x < 1 { x = 1 }
	if y < 2 { y = 2 }
	if x+w >= m.width { x = m.width - w - 1 }
	if y+h >= m.height { y = m.height - h - 1 }

	borderStyle := m.nodeBorderStyle(node.Role)
	if focused { borderStyle = borderStyle.Bold(true) }
	contentStyle := lipgloss.NewStyle().Foreground(ClrText)

	tc, hc, vc := "┌", "─", "│"
	if m.asciiMode { tc, hc, vc = "+", "-", "|" }
	...
	if l1n > w-2 { line1 = string([]rune(line1)[:w-2]) }
	if l1n < w-2 { line1 += strings.Repeat(" ", w-2-l1n) }
	...
	if node.Status == StatusWorking || node.Status == StatusThinking {
		...
		if l2n > w-2 { line2 = string([]rune(line2)[:w-2]) }
		if l2n < w-2 { line2 += strings.Repeat(" ", w-2-l2n) }
...
func (m NodeGraphModel) drawEdge(edge EdgeState, placeRune func(int,int,rune,lipgloss.Style,int), z int) {
	...
	if edge.Active { style = lipgloss.NewStyle().Foreground(ClrAccent).Bold(true) }
	if edge.Error { style = lipgloss.NewStyle().Foreground(ClrRose).Bold(true) }

	// Simple Manhattan routing: down then horizontal then down/up
	midY := (fy + ty) / 2
	for y := fy; y <= midY && y < m.height; y++ { placeRune(fx, y, '│', style, z) }
	if fx < tx { for x := fx; x <= tx && x < m.width; x++ { placeRune(x, midY, '─', style, z) } }
	if fx > tx { for x := fx; x >= tx && x >= 0; x-- { placeRune(x, midY, '─', style, z) } }
	for y := midY; y <= ty && y < m.height; y++ { placeRune(tx, y, '│', style, z) }
...
func (m NodeGraphModel) overlayPopup(base, popup string) string {
	...
	pw := 0
	for _, l := range popupLines { if len(l) > pw { pw = len(l) } }
	ph := len(popupLines)
	...
	for i, pl := range popupLines {
		...
		if by >= 0 && by < len(baseLines) {
			...
			if startX < len(bl) {
				left := ""
				if startX > 0 { left = bl[:startX] }
				right := ""
				if startX+len(pl) < len(bl) { right = bl[startX+len(pl):] }
...
func (m NodeGraphModel) nodeBorderStyle(role string) lipgloss.Style {
	switch role {
	case "orchestrator": return lipgloss.NewStyle().Foreground(ClrViolet)
	case "frontend": return lipgloss.NewStyle().Foreground(ClrCyan)
	case "backend": return lipgloss.NewStyle().Foreground(ClrGreen)
	case "database": return lipgloss.NewStyle().Foreground(ClrAmber)
	case "qa": return lipgloss.NewStyle().Foreground(ClrRose)
	default: return lipgloss.NewStyle().Foreground(ClrAccent)
	}
}

func (m NodeGraphModel) nodeBgStyle(role string) lipgloss.Style {
	switch role {
	case "orchestrator": return lipgloss.NewStyle().Background(ClrViolet).Foreground(ClrBase)
	case "frontend": return lipgloss.NewStyle().Background(ClrCyan).Foreground(ClrBase)
	case "backend": return lipgloss.NewStyle().Background(ClrGreen).Foreground(ClrBase)
	case "database": return lipgloss.NewStyle().Background(ClrAmber).Foreground(ClrBase)
	case "qa": return lipgloss.NewStyle().Background(ClrRose).Foreground(ClrBase)
	default: return lipgloss.NewStyle().Background(ClrElevated).Foreground(ClrText)
	}
}
...
func statusIndicator(status NodeStatus) string {
	switch status {
	case StatusIdle: return "○"
	case StatusWaiting: return "○ ○"
	case StatusWorking: return "▓▓▓"
	case StatusThinking: return "◐"
	case StatusDone: return "✓"
	case StatusError: return "✕"
	case StatusReview: return "◯"
	default: return "○"
	}
}
