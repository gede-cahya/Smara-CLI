...
const defaultAvatar: AvatarConfig = { enabled: true, model: 'smara-cyber-neko-placeholder', style: 'anime-3d-cyber-companion', state: 'idle', expression: 'soft-smile', speech_bubble: 'Halo, saya Smara. Siap membantu.', lip_sync: true, gesture: 'idle-float', light_mode: false, voice_reactive: true, intensity: 0.35 }
  ...
  return (
    <div className="h-full overflow-y-auto p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><div className="flex items-center gap-2 text-cyan-200 font-semibold"><Bot className="w-5 h-5" /> Anime 3D Character Assistant</div><p className="text-sm text-gray-400 mt-1">Phase 5 MVP: avatar anime 3D placeholder, state/expression sync, lip-sync TTS, gesture feedback, speech bubble, dan light-mode toggle.</p></div>
      ...
      <div className="grid xl:grid-cols-[1.15fr_.85fr] gap-5">
        <div className="relative min-h-[560px] rounded-3xl border border-white/10 bg-black/30 overflow-hidden">
          {cfg.enabled ? <Canvas camera={{ position: [0, 1.1, 4.2], fov: 45 }}><color attach="background" args={[cfg.light_mode ? '#111827' : '#020617']} /><ambientLight intensity={0.65} /><Stars radius={45} depth={25} count={900} factor={3} fade speed={0.25} /><SmaraAvatarModel cfg={cfg} /><OrbitControls enablePan={false} minDistance={2.5} maxDistance={6} /></Canvas> : <div className="h-[560px] flex items-center justify-center text-gray-500">Avatar dimatikan untuk mode ringan.</div>}
          <div className="absolute left-5 right-5 bottom-5 rounded-3xl border border-white/10 bg-black/55 backdrop-blur-xl p-4 flex gap-3"><MessageCircle className="w-5 h-5 text-fuchsia-200 shrink-0" /><div><div className="text-xs text-gray-500 mb-1">Speech bubble</div><div className="text-sm text-gray-100">{cfg.speech_bubble}</div></div></div>
        </div>
        <div className="space-y-4">
          ...
          <div className="rounded-3xl border border-white/10 bg-black/25 p-5 space-y-3">
            ...
            <textarea value={bubble} onChange={e => setBubble(e.target.value)} className="w-full h-24 rounded-2xl bg-black/40 border border-white/10 p-3 text-sm outline-none focus:border-cyan-300/40" />
            <div className="grid grid-cols-2 gap-2"><button onClick={() => postEvent({ speech_bubble: bubble, state: 'thinking' })} className="px-3 py-3 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-sm flex items-center justify-center gap-2"><RotateCcw className="w-4 h-4" /> Update bubble</button><button onClick={speakDemo} className="px-3 py-3 rounded-2xl bg-fuchsia-500/15 hover:bg-fuchsia-500/25 border border-fuchsia-300/20 text-sm flex items-center justify-center gap-2"><Volume2 className="w-4 h-4" /> Speak + lip sync</button><button onClick={() => postEvent({ state: 'acting', gesture: 'pointer-guide', speech_bubble: 'Saya mengarahkan Magic Pointer ke target UI.' })} className="px-3 py-3 rounded-2xl bg-cyan-500/15 hover:bg-cyan-500/25 border border-cyan-300/20 text-sm flex items-center justify-center gap-2"><MousePointer2 className="w-4 h-4" /> Pointer gesture</button><button onClick={() => postEvent({ state: 'emergency_stop', speech_bubble: 'Autopilot dihentikan untuk keamanan.' })} className="px-3 py-3 rounded-2xl bg-red-500/15 hover:bg-red-500/25 border border-red-300/20 text-sm flex items-center justify-center gap-2"><Power className="w-4 h-4" /> Emergency</button></div>
          ...
          <div className="rounded-3xl border border-white/10 bg-black/25 p-4 text-xs text-gray-400">Model saat ini adalah placeholder Three.js anime 3D cyber companion. Struktur sudah siap diganti VRM resmi Smara nanti.</div>
        </div>
      </div>
    </div>
  )
