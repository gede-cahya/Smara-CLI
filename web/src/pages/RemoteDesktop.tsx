...
export default function RemoteDesktop() {
  ...
  return <div className="h-full overflow-y-auto p-6 space-y-5">
    <div className="flex items-center justify-between">
      <div><div className="flex items-center gap-2 text-smara-200 font-semibold"><Monitor className="w-5 h-5" /> Remote Desktop Assistant</div>
    ...
    <div className="grid lg:grid-cols-[.9fr_1.2fr] gap-5">
      <div className="space-y-5">
        <div className="rounded-3xl border border-neutral-800/70 bg-slate-950/25 p-5 space-y-3">
          <div className="flex items-center gap-2 text-sm font-semibold"><Link2 className="w-4 h-4 text-lime-300" /> Pair desktop-agent</div>
          <input value={name} onChange={e=>setName(e.target.value)} className="w-full rounded-2xl bg-slate-950/40 border border-neutral-800/70 p-3 text-sm outline-none" placeholder="Device name" />
          <input value={url} onChange={e=>setUrl(e.target.value)} className="w-full rounded-2xl bg-slate-950/40 border border-neutral-800/70 p-3 text-sm outline-none" placeholder="http://host:8765" />
          <input value={token} onChange={e=>setToken(e.target.value)} className="w-full rounded-2xl bg-slate-950/40 border border-neutral-800/70 p-3 text-sm outline-none" placeholder="token opsional" type="password" />
          <button onClick={pair} className="w-full px-4 py-3 rounded-2xl bg-smara-500/20 hover:bg-smara-500/30 border border-smara-300/20 text-smara-100 text-sm">Pair / Update</button>
        ...
        <div className="rounded-3xl border border-neutral-800/70 bg-slate-950/25 p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={()=>proxy('health')} className="px-3 py-2 rounded-xl bg-white/8 border border-neutral-800/70 text-sm">Health</button>
            <button onClick={()=>proxy('observe')} className="px-3 py-2 rounded-xl bg-white/8 border border-neutral-800/70 text-sm flex items-center justify-center gap-2"><Camera className="w-4 h-4"/>Observe</button>
            ...
            <button onClick={()=>proxy('resume')} className="px-3 py-2 rounded-xl bg-emerald-400/15 border border-emerald-300/20 text-emerald-100 text-sm flex items-center justify-center gap-2"><RefreshCw className="w-4 h-4"/>Resume</button>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-5">
        <div className="rounded-3xl border border-neutral-800/70 bg-slate-950/25 p-5 space-y-3">
          <div className="flex items-center gap-2 text-sm font-semibold"><Play className="w-4 h-4 text-smara-300"/> Remote Magic Pointer task</div>
          <textarea value={instruction} onChange={e=>setInstruction(e.target.value)} className="w-full h-24 rounded-2xl bg-slate-950/40 border border-neutral-800/70 p-3 text-sm outline-none" />
          <input type="number" min={1} max={30} value={maxSteps} onChange={e=>setMaxSteps(Number(e.target.value))} className="w-full rounded-2xl bg-slate-950/40 border border-neutral-800/70 p-3 text-sm outline-none" />
          <button onClick={()=>proxy('task', { instruction, max_steps: maxSteps, assume_yes: true })} className="w-full px-4 py-3 rounded-2xl bg-gradient-to-r from-smara-500/25 to-lime-500/25 border border-smara-300/20 text-smara-50 text-sm">Run remote autopilot</button>
        </div>
