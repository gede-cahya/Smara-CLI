...
export default function MemoryGraph({ workspace }: { workspace?: string }) {
  ...
  return <div className="relative flex h-full overflow-hidden">
    <div className="flex-1 flex flex-col min-w-0">
      <div className="memory-graph-toolbar flex items-center gap-2 p-3 border-b border-smara-300/12 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-2 flex-wrap">
          ...
          <label className="flex items-center gap-1 text-xs text-gray-400">min w<input type="number" step="0.05" min="0" max="1" value={minWeight} onChange={e => setMinWeight(parseFloat(e.target.value) || 0)} className="memory-graph-control w-14 rounded px-1.5 py-1" /></label>
          ...
          <label className="flex items-center gap-1 text-xs text-gray-400">threshold<input type="number" step="0.01" min="0" max="1" value={threshold} onChange={e => setThreshold(parseFloat(e.target.value) || 0.78)} className="memory-graph-control w-14 rounded px-1.5 py-1" /></label>
          <label className="flex items-center gap-1 text-xs text-gray-400">top-k<input type="number" min="1" max="20" value={topK} onChange={e => setTopK(parseInt(e.target.value) || 5)} className="memory-graph-control w-12 rounded px-1.5 py-1" /></label>
          <select value={renderer} onChange={e => setRenderer(e.target.value as 'auto' | 'detail' | 'fast')} className="memory-graph-control rounded px-2 py-1 text-xs"><option value="auto">Auto renderer</option><option value="fast">Optimized Canvas</option><option value="detail">Detail ReactFlow</option></select>
          <select value={nodeLimit} onChange={e => setNodeLimit(parseInt(e.target.value) || 1000)} className="memory-graph-control rounded px-2 py-1 text-xs"><option value={300}>300 nodes</option><option value={1000}>1k nodes</option><option value={3000}>3k nodes</option><option value={10000}>10k nodes</option></select>
          <select value={edgeLimit} onChange={e => setEdgeLimit(parseInt(e.target.value) || 2500)} className="memory-graph-control rounded px-2 py-1 text-xs"><option value={800}>800 edges</option><option value={2500}>2.5k edges</option><option value={8000}>8k edges</option><option value={25000}>25k edges</option></select>
          <button onClick={runAutolink} disabled={autolinking} className="px-2.5 py-1.5 bg-smara-500 hover:bg-smara-400 text-[#17200f] disabled:opacity-50 rounded text-xs font-semibold flex items-center gap-1"><Sparkles className="w-3 h-3" />{autolinking ? 'Linking…' : 'Build Links'}</button>
          <button onClick={() => load()} disabled={loading} className="memory-graph-secondary-button px-2 py-1.5 disabled:opacity-50 rounded text-xs flex items-center gap-1"><RefreshCw className={`w-3 h-3 ${loading ? 'animate-spin' : ''}`} />Reload</button>
        </div>
      </div>
