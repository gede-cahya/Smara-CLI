...
export default function MemoryGraph({ workspace }: { workspace?: string }) {
  ...
  const load = useCallback(async (override?: { mode?: 'overview' | 'neighborhood' | 'search'; focusId?: number | null }) => {
    ...
    try {
      ...
      const params = new URLSearchParams({ mode, limit: String(nodeLimit), edge_limit: String(edgeLimit), min_weight: String(minWeight), auto: includeAuto ? '1' : '0', manual: includeManual ? '1' : '0' })
  ...
  }, [workspace, nodeLimit, edgeLimit, minWeight, includeAuto, includeManual, graphMode, focusNodeId, filter])
  ...
  const runAutolink = async () => {
    setAutolinking(true)
    try {
      const r = await fetchJSON<{ mode: 'semantic' | 'lexical' | 'none'; created: number; wikilinks_created?: number; memories_scanned: number; with_embedding: number; fell_back_to_lexical: boolean }>('/api/memories/autolink', { method: 'POST', body: JSON.stringify({ threshold, top_k: topK, replace: true, wikilinks: true }) })
  ...
  const stats = `${data.nodes.length}${data.meta?.total_nodes ? `/${data.meta.total_nodes}` : ''} nodes · ${data.edges.length}${data.meta?.total_edges ? `/${data.meta.total_edges}` : ''} edges · ${data.meta?.mode || graphMode}${useFastRenderer ? ' · fast LOD' : ''}`

  return <div className="relative flex h-full overflow-hidden">
    <div className="flex-1 flex flex-col min-w-0">
      <div className="flex items-center gap-2 p-3 border-b border-gray-800 bg-gray-900/40 flex-wrap">
        ...
        <div className="ml-4 flex items-center gap-1 bg-gray-800 border border-gray-700 rounded-md px-2"><Search className="w-3 h-3 text-gray-500" /><input value={filter} onChange={e => setFilter(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') runSearchGraph() }} placeholder="Filter/search node…" className="bg-transparent text-xs px-1 py-1.5 w-44 focus:outline-none" /><button onClick={runSearchGraph} className="text-[10px] text-cyan-300 hover:text-cyan-100">Search graph</button></div>
        <div className="ml-auto flex items-center gap-2 flex-wrap">
          {graphMode !== 'overview' && <button onClick={backToOverview} className="px-2 py-1.5 bg-cyan-900/40 hover:bg-cyan-800/50 rounded text-xs text-cyan-200">Overview</button>}
          <label className="flex items-center gap-1 text-xs text-gray-500">min w<input type="number" step="0.05" min="0" max="1" value={minWeight} onChange={e => setMinWeight(parseFloat(e.target.value) || 0)} className="w-14 bg-gray-800 border border-gray-700 rounded px-1.5 py-1 text-gray-300" /></label>
          ...
          <label className="flex items-center gap-1 text-xs text-gray-500">threshold<input type="number" step="0.01" min="0" max="1" value={threshold} onChange={e => setThreshold(parseFloat(e.target.value) || 0.78)} className="w-14 bg-gray-800 border border-gray-700 rounded px-1.5 py-1 text-gray-300" /></label>
          <label className="flex items-center gap-1 text-xs text-gray-500">top-k<input type="number" min="1" max="20" value={topK} onChange={e => setTopK(parseInt(e.target.value) || 5)} className="w-12 bg-gray-800 border border-gray-700 rounded px-1.5 py-1 text-gray-300" /></label>
          <select value={renderer} onChange={e => setRenderer(e.target.value as any)} className="bg-gray-800 border border-gray-700 rounded px-1.5 py-1 text-xs text-gray-300"><option value="auto">Auto renderer</option><option value="fast">Fast LOD</option><option value="detail">Detail ReactFlow</option></select>
          <select value={nodeLimit} onChange={e => setNodeLimit(parseInt(e.target.value) || 1000)} className="bg-gray-800 border border-gray-700 rounded px-1.5 py-1 text-xs text-gray-300"><option value={300}>300 nodes</option><option value={1000}>1k nodes</option><option value={3000}>3k nodes</option><option value={10000}>10k nodes</option></select>
          <select value={edgeLimit} onChange={e => setEdgeLimit(parseInt(e.target.value) || 2500)} className="bg-gray-800 border border-gray-700 rounded px-1.5 py-1 text-xs text-gray-300"><option value={800}>800 edges</option><option value={2500}>2.5k edges</option><option value={8000}>8k edges</option><option value={25000}>25k edges</option></select>
          <button onClick={runAutolink} disabled={autolinking} className="px-2.5 py-1.5 bg-smara-700 hover:bg-smara-600 disabled:opacity-50 rounded text-xs text-white flex items-center gap-1"><Sparkles className="w-3 h-3" />{autolinking ? 'Linking…' : 'Build Links'}</button>
      ...
      <div className="flex-1 relative overflow-hidden bg-[radial-gradient(circle_at_50%_42%,rgba(30,41,59,0.72),transparent_36%),radial-gradient(circle_at_20%_12%,rgba(45,212,191,0.08),transparent_28%),radial-gradient(circle_at_78%_24%,rgba(132,204,22,0.08),transparent_24%),linear-gradient(180deg,#020617,#060914_48%,#020617)]">
        ...
        <div className="absolute bottom-3 left-3 bg-slate-950/70 backdrop-blur-xl border border-white/10 rounded-xl px-3 py-2 text-[10px] text-gray-300 shadow-2xl"><div className="font-semibold text-[11px] text-gray-100 mb-1.5">Interactive Memory Graph · klik node highlight · double-click focus neighborhood</div><div className="flex flex-wrap gap-3"><span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-slate-400" /> node</span><span className="flex items-center gap-1.5"><span className="w-4 h-4 rounded-full bg-lime-300" /> hub</span><span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-yellow-300" /> selected</span><span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-cyan-300" /> neighbor</span></div></div>
        {selected && <div className="absolute top-4 right-4 w-96 max-w-[calc(100%-2rem)] bg-slate-950/85 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_22px_80px_rgba(0,0,0,0.55)] overflow-hidden z-20"><div className="p-4 border-b border-white/10 bg-gradient-to-r from-lime-400/10 via-cyan-400/10 to-transparent"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-[10px] uppercase tracking-[0.24em] text-lime-200/80">Memory #{selected.id}{selected.source ? ` · ${selected.source}` : ''}</div><h3 className="mt-1 text-sm font-semibold text-white leading-snug">{selected.label || shortText(selected.content, 70)}</h3></div><button onClick={() => { setSelected(null); setHighlightedId(null) }} className="p-1 rounded-lg text-gray-500 hover:text-white hover:bg-white/10"><X className="w-4 h-4" /></button></div><div className="mt-3 flex items-center gap-2 text-[11px] text-gray-300"><span className="px-2 py-1 rounded-full bg-lime-400/10 border border-lime-300/20 text-lime-200">degree {selected.degree}</span><span className="px-2 py-1 rounded-full bg-cyan-400/10 border border-cyan-300/20 text-cyan-200">{linksOfSelected.length} links</span><button onClick={() => focusNeighborhood(selected)} className="px-2 py-1 rounded-full bg-yellow-400/10 border border-yellow-300/20 text-yellow-200 hover:bg-yellow-400/20">focus</button></div></div><div className="p-4 space-y-3 max-h-[62vh] overflow-y-auto"><div className="flex flex-wrap gap-1.5">{(selected.tags || []).map((t, i) => <span key={i} className="text-[10px] bg-smara-700/30 text-smara-200 border border-smara-400/20 px-2 py-0.5 rounded-full"><Tag className="w-2.5 h-2.5 inline mr-0.5" />{t}</span>)}{(!selected.tags || selected.tags.length === 0) && <span className="text-[10px] text-gray-600">no tags</span>}</div><div className="bg-black/30 border border-white/10 rounded-xl p-3 text-xs leading-relaxed text-gray-200 whitespace-pre-wrap">{selected.content}</div><div className="pt-2 border-t border-white/10"><div className="text-[10px] text-gray-500 uppercase tracking-wider mb-2">Connected memories</div>{linksOfSelected.length === 0 && <div className="text-xs text-gray-600">Belum ada link.</div>}<div className="space-y-1.5">{linksOfSelected.map(l => { const otherId = l.source_id === selected.id ? l.target_id : l.source_id; const other = data.nodes.find(n => n.id === otherId); return <div key={l.id} className="flex items-center gap-2 text-xs bg-white/[0.03] border border-white/10 rounded-lg px-2 py-2 group"><span className={`text-[10px] px-1.5 py-0.5 rounded ${l.auto_linked ? 'bg-cyan-900/40 text-cyan-300' : 'bg-lime-900/40 text-lime-300'}`}>{l.relation}</span><span className="text-gray-500 text-[10px]">w={l.weight.toFixed(2)}</span><button className="flex-1 text-left text-gray-300 hover:text-lime-200 truncate" onClick={() => { if (other) { setSelected(other); setHighlightedId(other.id) } }} title={other?.content || `Memory #${otherId}`}>{other ? `[${other.id}] ${other.label || shortText(other.content, 40)}` : `[${otherId}] —`}</button><button onClick={() => removeLink(l.id)} className="opacity-0 group-hover:opacity-100 text-gray-600 hover:text-red-400 transition-opacity"><Trash2 className="w-3 h-3" /></button></div> })}</div></div></div></div>}
      </div>
    </div>
